...
namespace MiniPOW {
    ...
    //% blockId="set_offset" block="Set offset - |left %p1 %_cal_0|right %p2 %_cal_1|2x body %p3 %_cal_2"
    //% weight=90 inlineInputMode=inline
    export function set_offset(p1: AnalogPin, _cal_0: number, p2: AnalogPin, _cal_1: number, p3: AnalogPin, _cal_2: number): void {
        d[0] = _cal_0;
        d[1] = _cal_1;
        d[2] = _cal_2;
        pin1 = p1;
    ...
    function s90(s: number, angle: number) {
        if (s == 3) {
            pins.servoWritePin(pin1, angle + d[0])
        }
        if (s == 1) {
            pins.servoWritePin(pin2, angle + d[1])
        }
        if (s == 0) {
            pins.servoWritePin(pin3, angle + d[2])
        }
    }
